...
public class RGBImage implements ce326.hw2.Image, Cloneable {
    ...
    public void foursize() {// brightness of each pixel is copied in different positions
        RGBPixel[][] arrayhelp = new RGBPixel[4 * height][4 * width];
        ...
        for (int i = 0; i < height; i++) {
            for (int j = 0; j < width; j++) {
                arrayhelp[4 * i][4 * j] = array[i][j];
                arrayhelp[4 * i + 2][4 * j] = array[i][j];
                arrayhelp[4 * i][4 * j + 2] = array[i][j];
                arrayhelp[4 * i + 2][4 * j + 2] = array[i][j];
            }
        }
        array = arrayhelp.clone();
        height = 4 * height;
